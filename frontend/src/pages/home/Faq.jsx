...
const faqs = [
  ...
  },
  {
    ...
    answer:
      "Pengalaman Netflix Anak disertakan dalam keanggotaan agar kamu punya kontrol orang tua saat anak-anak menikmati acara TV dan film untuk keluarga di rumah. Profil anak dilengkapi kontrol orang tua yang dilindungi PIN sehingga kamu bisa membatasi rating usia konten yang bisa ditonton anak dan memblokir judul tertentu yang tidak boleh ditonton anak-anak.",
  },
];
...
const Faq = () => {
  ...
  return (
    <div className="bg-black text-white py-10 px-4 md:px-20">
      <h1 className="text-3xl font-bold mb-6">Tanya Jawab Umum</h1>
      <div className="space-y-2">
        {faqs.map((item, index) => (
          <div key={index} className="bg-[#303030] rounded overflow-hidden transition-all duration-300">
            <button
              onClick={() => toggle(index)}
              className="w-full flex justify-between items-center p-5 text-left text-lg font-medium focus:outline-none hover:bg-[#404040] transition-colors duration-300 cursor-pointer"
            >
              {item.question}
              {openIndex === index ? <X size={24} /> : <Plus size={24} />}
            </button>
            {openIndex === index && (
              <div className="px-5 pb-5 text-base border-t border-gray-700">
                {item.answer}
              </div>
            )}
          </div>
